...
func parseInput() (*truss.Config, error) {
	...
	p, err := loadPackage(nil, protoDir)
	if err != nil || len(p) == 0 {
		return nil, errors.Wrap(err, "proto files not found in importable go package")
	}

	cfg.PBPackage = p[0].PkgPath
	cfg.PBPath = changeOutDir(protoDir)
	// 判断服务路径是否存在, 不存在创建文件夹
	if !fileExists(cfg.PBPath) {
		if err := os.Mkdir(cfg.PBPath, 777); err != nil {
			return nil, errors.Wrap(err, "创建服务根文件夹失败")
		}
	}
	...
	if err != nil {
		...
		log.Info(".pb.go generation with protoc-enum_gen-go was successful.")
		return nil, nil
	}
	...
	cfg.ServicePath = changeOutDir(svcPath)
...
type DefinePackage struct {
	Name    string
	PkgPath string
}

// 将路径转换为包名
func loadPackage(place *string, path string) ([]DefinePackage, error) {
	root := rootPath()
	if root == "" {
		return nil, errors.New("未找到项目根目录,请配置 ZBE_PATH环境变量")
	}

	splits := strings.Split(path, "services")
	if len(splits) != 2 {
		return nil, errors.New("truss 的服务必须在services目录下")
	}

	goModule, err := getGoModulePath()
	if err != nil {
		return nil, err
	}

	p := DefinePackage{}
	p.PkgPath = goModule + "/services"
	p.PkgPath += strings.ReplaceAll(splits[1], "\\", "/")
	// 名称取最后一个
	splits = strings.Split(p.PkgPath, "/")
	p.Name = splits[len(splits)-1]

	return []DefinePackage{p}, nil
}

// 调整pb和服务的输出目录
func changeOutDir(dir string) string {
	splits := strings.Split(dir, "services")
	if len(splits) != 2 {
		return ""
	}

	root := rootPath()
	p := path.Join(root, "services")
	p += strings.ReplaceAll(splits[1], "\\", "/")
	return p
}

func rootPath() string {
	return os.Getenv("ZBE_PATH")
}

func getGoModulePath() (string, error) {
	root := rootPath()
	fullPath := path.Join(root, "configs", "truss.yaml")
...
func readPreviousGeneration(serviceDir string) (map[string]io.Reader, error) {
	...
	const transportHttpDirName = "transport_http"
	...
	addFileToFiles := func(path string, info os.FileInfo, err error) error {
		if info.IsDir() {
			...
			// See `gengokit/generator/enum_gen.go:generateResponseFile`
			case filepath.Base(serviceDir), transportHttpDirName, handlersDirName:
				return nil
			default:
	...
	// 输出 transport_http目录( 不重复生成 )
	transportHttpDir := path.Join(serviceDir, "svc", transportHttpDirName)
	// 如果文件不存在,需要新建目录
	if !fileExists(transportHttpDir) {
		err = os.MkdirAll(transportHttpDir, 0777)
	}
	err = filepath.Walk(transportHttpDir, addFileToFiles)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot fully walk directory %v", serviceDir)
	}
...
// promptNoMake prints that truss was not built with make and prompts the merchant
