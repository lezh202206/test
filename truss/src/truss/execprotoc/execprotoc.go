...
func GeneratePBDotGo(protoPaths, gopath []string, outDir string) error {
	...
	_, err := exec.LookPath("protoc-enum_gen-gogo")
	if err != nil {
		return errors.Wrap(err, "cannot find protoc-enum_gen-gogo in PATH")
	}

	err = protoc(protoPaths, gopath, genGoCode)
	if err != nil {
		return errors.Wrap(err, "cannot exec protoc with protoc-enum_gen-gogo")
	}
...
// have UTF-8 in the output. This caused protoc-enum_gen-truss-protocast to fail to
// output its the protoc AST.
func getProtocOutput(protoPaths, gopath []string) ([]byte, error) {
	_, err := exec.LookPath("protoc-enum_gen-truss-protocast")
	if err != nil {
		return nil, errors.Wrap(err, "protoc-enum_gen-truss-protocast does not exist in $PATH")
	}
