...
import (
	"bytes"
	"github.com/metaverse/truss/gengokit/svc/transport_http"
	"go/format"
	"io"
	"io/ioutil"
	"strings"
...
func generateResponseFile(templFP string, data *gengokit.Data, prevFile io.Reader) (io.Reader, error) {
	...
	case transport_http.PermissionsHttpPath:
		permissions := transport_http.NewPermissions()
		permissions.Load(prevFile)
		if genCode, err = permissions.Render(templFP, data); err != nil {
			return nil, errors.Wrapf(err, "cannot render template: %s", templFP)
		}
	case transport_http.TransportHttpPath:
		_transport := transport_http.NewTransportHttp()
		_transport.Load(prevFile)
		if genCode, err = _transport.Render(templFP, data); err != nil {
			return nil, errors.Wrapf(err, "cannot render template: %s", templFP)
		}
	default:
