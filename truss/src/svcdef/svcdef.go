...
Additionally, since svcdef only parses Go code generated by protoc-enum_gen-go, all
methods accept only ast types with structures created by protoc-enum_gen-go. See
			...
			case *ast.InterfaceType:
				...
				if !strings.HasSuffix(t.Name.Name, "Server") {
					...
					if !strings.HasSuffix(t.Name.Name, "Client") {
						...
						// field, which isn't currently supported.  Warn the merchant and skip.
...
// protoc-enum_gen-go. Those conventions are:
//
...
// code generated by protoc-enum_gen-go, an error will be returned.
